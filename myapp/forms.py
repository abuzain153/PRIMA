...
class ProductForm(forms.ModelForm):
        ...
        if selected_warehouses:
            ...
            if self.instance.pk:
                ...
                if self.instance.quantity == self.instance.quantity.to_integral_value():
                    ...
                    self.fields['quantity'].initial = format(self.instance.quantity, '.4f')
    ...
    def clean_quantity(self):
        return self.cleaned_data.get('quantity', Decimal('0.0000'))
...
class WithdrawQuantityFromWarehousesForm(forms.Form):
    ...
    def clean(self):
        cleaned_data = super().clean()
        total_withdrawn = Decimal('0.0000')
...
class AddQuantityToWarehousesForm(forms.Form):
    total_quantity_to_add = forms.DecimalField(
        ...
        decimal_places=4,
        required=True,
        widget=forms.NumberInput(attrs={'class': 'form-control', 'readonly': 'readonly', 'style': 'width: 150px; background-color: #e3f2fd;', 'step': 'any'})
    )
    product_id = forms.IntegerField(widget=forms.HiddenInput())
    ...
    def clean(self):
        ...
        for name, value in cleaned_data.items():
            if name.startswith('warehouse_') and value is not None:
